...
<h2>▶️ Быстрый запуск (Dev)</h2>

<h3>1) Поднять PostgreSQL</h3>

<pre><code>docker compose up -d
</code></pre>

<h3>2) Запустить приложение</h3>

<pre><code>mvn spring-boot:run
</code></pre>

<p>По умолчанию приложение стартует на <code>http://localhost:8080</code>.</p>
...
<p>При первом запуске (когда таблица <code>users</code> пустая) создаётся bootstrap-admin:</p>
<ul>
  <li>username: <code>admin</code></li>
  <li>password: <code>admin</code></li>
</ul>
<p>Переопределяется переменными окружения:</p>
<ul>
  <li><code>BOOTSTRAP_ADMIN_USERNAME</code></li>
  <li><code>BOOTSTRAP_ADMIN_PASSWORD</code></li>
</ul>

<h2>📌 Основные эндпоинты</h2>

<ul>
  <li><code>POST /api/auth/login</code> — получить JWT</li>
  <li><code>POST /api/admin/users</code> — создать пользователя (ADMIN)</li>
  <li><code>POST /api/admin/cards</code> — создать карту (ADMIN)</li>
  <li><code>GET /api/cards</code> — список своих карт (USER)</li>
  <li><code>POST /api/cards/{id}/block-request</code> — запрос блокировки (USER)</li>
  <li><code>POST /api/transfers</code> — перевод между своими картами (USER)</li>
</ul>
